...
const csvHeaders = [
    { label: "Youth First Name", key: "firstName" },
    { label: "Youth Last Name", key: "lastName" },
    { label: "Parent/Guardian Name", key: "parentName" },
    { label: "Parent Email", key: "parentEmail" },
    { label: "Parent Phone", key: "parentPhone" },
    { label: "Programs Interested", key: "programsInterested" },
...
function YouthInquiries() {
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this youth inquiry?")) {
            try {
                const response = await fetch('/delete-youth-inquiry-by-id', {
                    method: 'POST',
        ...
        <div className="container mt-5">
            ...
            <div className="d-flex flex-column align-items-center">
                ...
                <div className="w-auto mb-4 px-3 d-flex justify-content-between align-items-center" style={{ minWidth: '350px' }}>
                    <h2 className="mb-0 me-5">Youth Inquiries</h2>
                    <div className="d-flex gap-2">
                        {inquiries.length > 0 && (
                            <CSVLink
                                data={inquiries}
                                headers={csvHeaders}
                                filename={"508CHEERS_Youth_Inquiries.csv"}
                ...
                <div className="row justify-content-center w-100">
                    <div className="col-md-10">
                                ...
                                <div className="card-body">
                                    <div className="row text-start">
                                        <div className="col-md-4 mb-3 mb-md-0">
                                            <h6 className="text-muted small text-uppercase">Interests</h6>
                                            <div><strong>Programs:</strong> {inquiry.programsInterested}</div>
                                        </div>

                                        <div className="col-md-4 mb-3 mb-md-0">
                                            <h6 className="text-muted small text-uppercase">Parent/Guardian</h6>
                                            <div><strong>Name:</strong> {inquiry.parentName}</div>
                                            <div><strong>Email:</strong> {inquiry.parentEmail}</div>
                                            <div><strong>Phone:</strong> {inquiry.parentPhone}</div>
                                        </div>

                                        <div className="col-md-4">
                                            <h6 className="text-muted small text-uppercase">Permissions</h6>
                                            <div><strong>Parent Consent:</strong> {inquiry.consent ? "✅" : "❌"}</div>
                                        </div>
                                    </div>
                                    <hr/>
                                    <ExpandableText
                        ...
                        {inquiries.length === 0 && !error && (
                            <p className="text-center text-muted mt-5">Youth Inquiries database is empty.</p>
                        )}
                    </div>
                </div>
            </div>
